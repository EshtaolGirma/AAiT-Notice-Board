...
module.exports ={
    //GET ALL    
        getAll:(req, res) =>{
            NewsFeed.find().populate("deptId").exec()
            .then(result =>{
                console.log(result);
                res.status(200).json({
    ...
    //GET BY ID
        getOne: (req, res)=>{
            ...
            NewsFeed.find({_id:id}).populate("deptId").exec()
            .then(result =>{
                if(result){
                    console.log(result)
                    res.status(200).json({
    ...
    //POST
     ...
     //UPDATE   
        put: (req, res)=>{
            ...
            NewsFeed.findByIdAndUpdate({ _id: req.params.newsId},
                { $set: req.body})
            .exec()
            .then(result =>{
                console.log(result);
